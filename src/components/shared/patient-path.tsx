...
export function PatientPath({ steps }: PatientPathProps) {
  return (
    <ol className="relative isolate grid items-stretch gap-5 pl-8.5 lg:grid-cols-5 lg:pl-0">
      <span
        aria-hidden="true"
        data-pulse=""
      ...
      <span
        aria-hidden="true"
        data-pulse=""
        className="vessel-line animate-vpulse pointer-events-none absolute top-2 bottom-2 left-3.25 -z-10 w-0.75 lg:hidden"
      />
      ...
      {steps.map((item) => (
        ...
        >
          <span aria-hidden="true" className="vessel-node absolute -left-7.5 top-4.5 lg:hidden" />
